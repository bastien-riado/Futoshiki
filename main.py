...
def remplacer_boutton_grille(root):
    """
    Met à jour les bouton graphique de la grille.
    """
...
def initialiser_grille(root, taille):
    """
    Initialise une grille vide de la taille selectionnee et l'affiche dans la fenetre graphique principale.
    """
...
def charger_grille(root, IntSize):
    """
    Ouvre une fenetre de selection de fichier et importe le fichier selectionne dans la fenetre graphique.
    """
...
def sauvegarder_grille():
    """
    Ouvre une fenetre permettant de selectionner ou créer un nouveau fichier et exporte 
    la grille de la fenetre graphique pricipale dans ce fichier.
    """
...
def afficher_nouvelle_grille(IntSize, root):
    """
    Recupere la taille selectionner et fait appel a la fonction initialiser_grille() avec cette taille
    """
    global taille
    taille = (IntSize.get())
...
def main():
    """
    Fonction principale qui affiche l'interface graphique et initialiser les données.
    """
    ...
    verifier = Button(boutton_canvas,
                      text='Verifier', command=print_dimacs)
    ...
    quitter = Button(boutton_canvas,
                     text='Quitter', command=root.destroy)
...
def afficher_solution(solution, grille):
    ...
    style.configure('TLabel', font=('calibri', 20, 'bold'),
                    foreground="white", background="#535353", pady=(0.25, 0.25), padx=(0.25, 0.25))
    ####################

    if solution == []:
        box = Label(
            solutionW, text="Il n'y a pas de solution.")
        box.pack()
        return
    box = Label(
    ...
    canvas.configure(bg="#535353", borderwidth=0, highlightthickness=0)
    canvas.pack(side='bottom', expand=1)

    style.configure('A.TLabel', font=('calibri', int(20/(len(grille)/15)), 'bold'),
                    foreground="green", background="#535353", pady=(0.25, 0.25), padx=(0.25, 0.25))
    style.configure('B.TLabel', font=('calibri', int(20/(len(grille)/15)), 'bold'),
                    foreground="#333333", background="#535353", pady=(0.25, 0.25), padx=(0.25, 0.25))
...
def update_grille():
    """
    Met a jour le tableau grille avec le contenue des boutons de la grille graphique.
    """
...
def print_dimacs():
    """
    Enregistre dans le fichier clauses.dimacs les clauses de la grille contenue dans la fenetre d'interface graphique.
    """
